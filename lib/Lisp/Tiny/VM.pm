...
#             return $val;
#         }
#         if ($exp->[0]->val =~ /defvar/) { # (defvar var exp)
#             my $var = $exp->[1];
#             $self->{global}{$var} = $self->eval($exp->[2]);
#             return $self->{global}{$var};
#         }
#         if (my $fn = $builtin->{$exp->[0]->val}) {
#             my @args;
#             for (@$exp[1..$#$exp]) {
#                 push @args, $self->eval(
#                     ref($_) =~ /Lisp::Tiny::Symbol/ ?
#                         $_->val : $_
#                 );
#             }
#             return $fn->(@args);
#         }
#         return $self->{local}{$exp->val} if $self->{local}{$exp->val};
#         return $self->{global}{$exp->val};
#     }
# DIE:
#     die "error:". Dumper $exp;
# }
